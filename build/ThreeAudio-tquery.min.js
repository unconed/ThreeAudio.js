...
FFT.prototype.forward=function(a){var c=this.bufferSize,f=this.cosTable,e=this.sinTable,d=this.reverseTable,n=this.real,k=this.imag;if(Math.pow(2,Math.floor(Math.log(c)/Math.LN2))!==c)throw"Invalid buffer size, must be a power of 2.";if(c!==a.length)throw"Supplied buffer is not the same size as defined FFT. FFT Size: "+c+" Buffer Size: "+a.length;var g=1,m,r,p,q,y,h;for(h=0;h<c;h++)n[h]=a[d[h]],k[h]=0;for(;g<c;){a=f[g];d=e[g];m=1;for(var u=r=0;u<g;u++){for(h=u;h<c;)p=h+g,q=m*n[p]-r*k[p],y=m*k[p]+
r*n[p],n[p]=n[h]-q,k[p]=k[h]-y,n[h]+=q,k[h]+=y,h+=g<<1;h=m;m=h*a-r*d;r=h*d+r*a}g<<=1}return this.calculateSpectrum()};
FFT.prototype.inverse=function(a,c){var f=this.bufferSize,e=this.cosTable,d=this.sinTable,n=this.reverseTable;a=a||this.real;c=c||this.imag;var k=1,g,m,r,p,q,y,h;for(h=0;h<f;h++)c[h]*=-1;g=new Float32Array(f);m=new Float32Array(f);for(h=0;h<a.length;h++)g[h]=a[n[h]],m[h]=c[n[h]];a=g;for(c=m;k<f;){n=e[k];g=d[k];m=1;for(var u=r=0;u<k;u++){for(h=u;h<f;)p=h+k,q=m*a[p]-r*c[p],y=m*c[p]+r*a[p],a[p]=a[h]-q,c[p]=c[h]-y,a[h]+=q,c[h]+=y,h+=k<<1;h=m;m=h*n-r*g;r=h*g+r*n}k<<=1}e=new Float32Array(f);for(h=0;h<f;h++)e[h]=
a[h]/f;return e};
...
RFFT.prototype.forward=function(a){var c=this.bufferSize,f=this.spectrum,e=this.trans,d=2*Math.PI,n=Math.sqrt,k=c>>>1,g=2/c,m,r,p,q,y,h,u,t,z,w,v,E,F,G,I,B,C,D,J,K,L;this.reverseBinPermute(e,a);q=0;for(var A=4;q<c;A*=4){for(var x=q;x<c;x+=A)B=e[x]-e[x+1],e[x]+=e[x+1],e[x+1]=B;q=2*(A-1)}a=2;for(p=c>>>1;p>>>=1;){q=0;a<<=1;A=a<<1;m=a>>>2;r=a>>>3;do{if(1!==m)for(x=q;x<c;x+=A)t=x,z=t+m,w=z+m,v=w+m,q=e[w]+e[v],e[v]-=e[w],e[w]=e[t]-q,e[t]+=q,t+=r,z+=r,w+=r,v+=r,q=e[w]+e[v],y=e[w]-e[v],q=-q*Math.SQRT1_2,
y*=Math.SQRT1_2,B=e[z],e[v]=q+B,e[w]=q-B,e[z]=e[t]-y,e[t]+=y;else for(x=q;x<c;x+=A)t=x,z=t+m,w=z+m,v=w+m,q=e[w]+e[v],e[v]-=e[w],e[w]=e[t]-q,e[t]+=q;q=(A<<1)-a;A<<=2}while(q<c);L=d/a;for(var H=1;H<r;H++){C=H*L;D=Math.sin(C);C=Math.cos(C);J=4*C*(C*C-.75);K=4*D*(.75-D*D);q=0;A=a<<1;do{for(x=q;x<c;x+=A)t=x+H,z=t+m,w=z+m,v=w+m,E=x+m-H,F=E+m,G=F+m,I=G+m,y=e[G]*C-e[w]*D,q=e[G]*D+e[w]*C,u=e[I]*J-e[v]*K,h=e[I]*K+e[v]*J,B=y-u,y+=u,u=B,e[I]=y+e[F],e[w]=y-e[F],B=h-q,q+=h,h=B,e[v]=h+e[z],e[G]=h-e[z],e[F]=e[t]-
q,e[t]+=q,e[z]=u+e[E],e[E]-=u;q=(A<<1)-a;A<<=2}while(q<c)}}for(;--k;)d=e[k],a=e[c-k-1],d=g*n(d*d+a*a),d>this.peak&&(this.peakBand=k,this.peak=d),f[k]=d;f[0]=g*e[0];return f};(function(a){for(var c in a)if(!window[c])throw"Error: ThreeAudio requires "+a[c];})({THREE:"Three.js",MicroEvent:"MicroEvent.js"});window.ThreeAudio=window.ThreeAudio||{};ThreeAudio.getShader=function(a){var c=document.getElementById(a);return c&&(c.innerText||c.textContent)||a};window._=window._||{};
...
ThreeAudio.GridGeometry=function(a,c,f,e,d){var n=a.source;e=(e||n.samples)-1;d=Math.min(a.history,d||a.history)-1;var k=d/a.history,g=.5/(n.samples-1);offsetV=.5/a.history;a=new THREE.PlaneGeometry(c,f,e,d);_.each(a.faceVertexUvs[0],function(a){_.each(a,function(a){a.u+=g;a.v=1-a.v*k+offsetV})});return a};ThreeAudio.LevelDetect=function(a){this.data=a;this.levels=[[0,0,0,0]];this.smooth=[0,0,0,0];this.change=[0,0,0,0];a.levels={direct:this.levels[0],smooth:this.smooth,change:this.change}};
ThreeAudio.LevelDetect.prototype.analyse=function(){for(var a=this.data,c=this.levels,f=this.smooth,e=this.change,d=[0,0,0,0],n=[a.time,a.filter.bass,a.filter.mid,a.filter.treble],a=0;4>a;++a){for(var k=a,g=n[a],m=g.length,r=0,p=0;p<m;++p)var q=(g[p]-128)/128,r=r+q*q;d[k]=Math.sqrt(r/m)}c.unshift(d);7<c.length&&c.pop();n=[1,2,1];k=4;g=Math.min(c.length,n.length);for(m=0;4>m;++m){for(a=d=0;a<g;++a)d+=(c[a]&&c[a][m]||0)*n[a];f[m]=d/k}n=[1,3,2,-2,-3,-1];k=1;g=Math.min(c.length,n.length);for(m=0;4>m;++m){for(a=
d=0;a<g;++a)d+=(c[a]&&c[a][m]||0)*n[a];e[m]+=.5*(d/k-e[m])}this.data.levels.direct=this.levels[0]};var __taDebug=!1;
...
d=this.fft,n=this.n,k=this.history,g=this.histogram,m=this.histogramSorted,r=this;c.beat.missed=!1;c.beat.maybe=!1;c.beat.is=!1;c.beat.predicted=!1;c.beat.locked=!1;c.beat.adjusted=!1;this.beat?(c.beat.confidence=this.beat.confidence,c.beat.permanence=this.beat.permanence,c.beat.bpm=this.beat.bpm):(c.beat.confidence=0,c.beat.permanence=0,c.beat.bpm="");var p=f.direct[0];this.background+=.2*(p-this.background);this.energy+=.4*(p-this.energy);this.signal=p=(this.energy-this.background)/(1-this.background)*
3;f=this.maybe;maybe=p;this.maxMaybe=Math.max(.99*this.maxMaybe,maybe);this.maybe=maybe=maybe/Math.max(.2,this.maxMaybe)-.7;this.sample=p;for(k.unshift(p);k.length>n;)k.pop();e.set(k);d.forward(e);for(var q=this.spectrum=d.real,p=d.real,e=d.imag,d=0;d<n;++d)q[d]=p[d]*p[d]+e[d]*e[d];e=0;for(d=2;d<n/8;++d)e=Math.max(e,q[d]);for(var k={},y=e/16,d=this.fMin+1;d<this.fMax;++d)if(p=q[d],p>y&&p>Math.max(q[d-1],q[d+1])&&.5<Math.abs(p-Math.min(q[d-1],q[d+1]))/p){var h=Math.sqrt(p/e);k[d]=h}_.each(m,function(a,
c){a.permanence*=.99;decay=Math.min(1,Math.log(1+a.permanence)/Math.log(10));a.strength*=decay;a.active=!1});_.each(k,function(a,c){c=+c;var d=q[c-1],e=q[c+1],f=d+e-2*q[c];b=(e-d)/2;fraction=-b/f;d=g[c];d||((d=g[c+1])?(delete g[c+1],d.index=c,d.fraction+=1,g[c]=d):(d=g[c-1])?(delete g[c-1],d.index=c,--d.fraction,g[c]=d):(d=g[c]={index:c,fraction:fraction,offset:0,strength:0,permanence:0,score:0,window:0},m.push(d)));d.fraction+=.1*(fraction-d.fraction);d.strength=a;d.permanence+=.01*d.strength;d.offset=
d.index+d.fraction;d.bpm=r.offsetToBPM(d.offset);d.window=n/d.offset;d.active=!0});m.sort(function(a,c){return c.permanence-a.permanence});for(var u;(u=m[m.length-1])&&.01>u.strength;)m.splice(m.length-1,1),delete g[u.index];var t=this.fMin,z=0,w=0,v=null;_.each(m,function(c){var d=0;if(!(c.offset<t)){var e=a(c);e>d&&(d=e);d+=c.strength*c.permanence;c.score=d;d>z?(v=c,w=z,z=d):d>w&&(w=d)}});v&&(v.score=z?1-w/z:0,v.confidence=Math.min(1,v.score+v.permanence),this.beat?this.beat.confidence<v.confidence?
...
e=this.signal,d=this.maybe,n=this.spectrum,k=this.histogram,g=this.data.beat,m=this,r=this.n;if(this.beat){var p=this.beat,q=1.5*p.offset;_.each(k,function(a){var c=a==p?1:0;a.offset>q&&(c=a.offset/p.offset,c=Math.max(0,1-4*Math.abs(c-Math.round(c))));a.match=c})}var y=["<strong><span"+(g.locked?' style="color: rgb(180,255,0)"':"")+">"+Math.round(10*g.bpm)/10+" BPM </span> ("+Math.round(100*g.confidence)+"%) P:"+Math.round(100*g.permanence)+" \u00b5 = "+Math.round(10*this.mean)/10+" \u03c3 = "+Math.round(100*
this.stddev)/100+"</strong> "+this.found+"f "+this.missed+"m"];_.each(k,function(a){var c=Math.round(10*m.offsetToBPM(a.offset))/10;y.push([c," bpm - ",Math.round(100*a.strength),"% P: ",Math.round(100*a.permanence)].join(""))});this.t.innerHTML=y.join("<br>");var h=this.g;h.fillStyle="#000000";h.fillRect(0,169,512,102);for(var u=0,t=2;t<r/8;++t)u=Math.max(n[t],u);u=1/u;h.beginPath();h.moveTo(0,270);for(t=2;t<r/8;++t)h.lineTo(8*(t-2),270-n[t]*u*100);h.strokeStyle="#ffffff";h.stroke();_.each(k,function(a){var c=
