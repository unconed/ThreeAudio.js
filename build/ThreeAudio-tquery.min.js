...
FFT.prototype.forward=function(a){var c=this.bufferSize,j=this.cosTable,d=this.sinTable,e=this.reverseTable,g=this.real,f=this.imag,k=Math.floor(Math.log(c)/Math.LN2);if(Math.pow(2,k)!==c)throw"Invalid buffer size, must be a power of 2.";if(c!==a.length)throw"Supplied buffer is not the same size as defined FFT. FFT Size: "+c+" Buffer Size: "+a.length;var k=1,m,n,q,p,t,h;for(h=0;h<c;h++)g[h]=a[e[h]],f[h]=0;for(;k<c;){a=j[k];e=d[k];m=1;for(var r=n=0;r<k;r++){for(h=r;h<c;)q=h+k,p=m*g[q]-n*f[q],t=m*f[q]+
n*g[q],g[q]=g[h]-p,f[q]=f[h]-t,g[h]+=p,f[h]+=t,h+=k<<1;h=m;m=h*a-n*e;n=h*e+n*a}k<<=1}return this.calculateSpectrum()};
FFT.prototype.inverse=function(a,c){var j=this.bufferSize,d=this.cosTable,e=this.sinTable,g=this.reverseTable,a=a||this.real,c=c||this.imag,f=1,k,m,n,q,p,t,h;for(h=0;h<j;h++)c[h]*=-1;k=new Float32Array(j);m=new Float32Array(j);for(h=0;h<a.length;h++)k[h]=a[g[h]],m[h]=c[g[h]];a=k;for(c=m;f<j;){g=d[f];k=e[f];m=1;for(var r=n=0;r<f;r++){for(h=r;h<j;)q=h+f,p=m*a[q]-n*c[q],t=m*c[q]+n*a[q],a[q]=a[h]-p,c[q]=c[h]-t,a[h]+=p,c[h]+=t,h+=f<<1;h=m;m=h*g-n*k;n=h*k+n*g}f<<=1}d=new Float32Array(j);for(h=0;h<j;h++)d[h]=
a[h]/j;return d};
...
document.body.appendChild(this.t)},bpmToOffset:function(a){return this.n*a/3600},offsetToBPM:function(a){return 3600*a/this.n},analyse:function(){var a=this.data,c=a.levels,j=this.buffer,d=this.fft,e=this.n,g=this.history,f=this.histogram,k=this.histogramSorted,m=this;a.beat.missed=!1;a.beat.maybe=!1;a.beat.is=!1;a.beat.predicted=!1;a.beat.locked=!1;a.beat.adjusted=!1;this.beat&&(a.beat.confidence=this.beat.confidence,a.beat.permanence=this.beat.permanence,a.beat.bpm=this.beat.bpm);var n=c.direct[0];
this.background+=0.2*(n-this.background);this.energy+=0.4*(n-this.energy);this.signal=n=3*((this.energy-this.background)/(1-this.background));c=this.maybe;maybe=2*n-this.signal;this.maxMaybe=Math.max(0.99*this.maxMaybe,maybe);this.maybe=maybe=maybe/Math.max(0.2,this.maxMaybe)-0.7;this.sample=n;for(g.unshift(n);g.length>e;)g.pop();j.set(g);d.forward(j);for(var q=this.spectrum=d.real,n=d.real,j=d.imag,d=0;d<e;++d)q[d]=n[d]*n[d]+j[d]*j[d];j=0;for(d=2;d<e/8;++d)j=Math.max(j,q[d]);for(var g={},p=j/16,
...
this.missed);this.decay+=0.4*(2.5*+a.beat.is-this.decay);a.beat.was+=0.4*(2.5*this.decay-a.beat.was);this.debounceMaybe++;this.debouncePredict++;this.measure++;this.frames++;__taDebug&&this.debug()},debug:function(){function a(a){a=Math.round(Math.max(0,Math.min(255,255*a)));h.fillStyle="rgb("+[a,a,a].join()+")";h.fillRect(s,x,1,20);x+=20}var c=this.data.levels.direct,j=this.sample,d=this.signal,e=this.maybe,g=this.spectrum,f=this.histogram,k=this.data.beat,m=this,n=this.n;if(this.beat){var q=this.beat,
...
10;t.push([c," bpm - ",Math.round(100*a.strength),"% P: ",Math.round(100*a.permanence)].join(""))});this.t.innerHTML=t.join("<br>");var h=this.g;h.fillStyle="#000000";h.fillRect(0,140,512,100);for(var r=0,s=2;s<n/8;++s)r=Math.max(g[s],r);r=1/r;h.beginPath();h.moveTo(0,200);for(s=2;s<n/8;++s)h.lineTo(8*(s-2),240-100*g[s]*r);h.strokeStyle="#ffffff";h.stroke();_.each(f,function(a){var c=0.75*a.strength+0.25,d=a.active?[Math.round(255-195*a.match),Math.round(180+40*a.match),0].join():"255,10,10";h.fillStyle=
"rgba("+d+","+c+")";h.fillRect(8*(a.offset-2),140,1,100)});var s=this.i,x=0;a(c[1]);a(c[2]);a(c[3]);a(0);k.is&&(h.fillStyle=k.missed?"rgba(255,0,0,.5)":k.maybe?"rgba(0,180,255,.75)":k.predicted?"rgba(255,180,0,.75)":"rgba(60,220,0,.75)",h.fillRect(this.i,0,2,100));c=Math.round(Math.max(0,Math.min(255,255*k.was)));h.fillStyle="rgb("+c+","+c+","+c+")";h.fillRect(412,240,100,100);k.maybe&&!k.is&&(h.fillStyle="rgba(64,64,64,.75)",h.fillRect(this.i,0,2,100));j=Math.floor(255*Math.max(0,Math.min(1,j+0.5)));
h.fillStyle="rgba("+Math.round(0.7*j)+","+Math.round(0.8*j)+","+j+",1)";h.fillRect(this.i,80,1,20);d=Math.floor(255*Math.max(0,Math.min(1,2*d)));h.fillStyle="rgba("+d+","+Math.round(0.8*d)+","+Math.round(0.5*d)+",1)";h.fillRect(this.i,100,1,20);e=k.is||k.maybe?Math.floor(255*Math.max(0,Math.min(1,e+0.5))):0;h.fillStyle="rgba("+Math.round(0.9*e)+","+e+","+Math.round(0.5*e)+",1)";h.fillRect(this.i,120,1,20);this.i=(s+1)%512}};
tQuery.World.register("audio",function(a){return tQuery.createAudioSource(this,a)});tQuery.register("createAudioSource",function(a,c){var j=new ThreeAudio.Source(c);j.textures=function(c){return tQuery.createAudioTextures(a,this,c)};return j});tQuery.register("createAudioTextures",function(a,c,j){var d=new ThreeAudio.Textures(a.tRenderer(),c,j);d.material=function(a,c,d,j,m){return tQuery.createAudioMaterial(this,a,c,d,j,m)};a.loop().hookPreRender(function(){d.update()});return d});
tQuery.register("createAudioMaterial",function(a,c,j,d,e,g){c=new ThreeAudio.Material(a,c,j,d,e,g);c.grid=function(c,d,e,g){return tQuery.createAudioGrid(a,c,d,e,g,this)};return c});tQuery.register("createAudioGrid",function(a,c,j,d,e,g){var f=[a,1,1,0,0];for(i in f)arguments[i]=arguments[i]||f[i];f=new ThreeAudio.GridGeometry(a,c||1,j||1,d,e);g=g||tQuery.defaultObject3DMaterial;f=new THREE.Mesh(f,g);f.doubleSided=!0;f.frustumCulled=!1;return tQuery(f)});
