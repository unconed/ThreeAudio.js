...
this.levelDetect.analyse();this.beatDetect.analyse();return this},size:function(){return this.analyser.frequencyBinCount},load:function(a,c){var d=this.context,h=this.source,f=this,e=new XMLHttpRequest;e.open("GET",a,true);e.responseType="arraybuffer";e.onload=function(){var a=d.createBuffer(e.response,false);h.buffer=a;h.loop=true;f.playing&&f._play();c&&c()};e.send();return this},play:function(){this.playing=true;this.source.buffer&&this._play();return this},stop:function(){this.playing=false;this.source.buffer&&
...
ThreeAudio.Textures.prototype={init:function(){var a=this.renderer,c=a.getContext(),d=this.textures,h=this.data,f=this.history,e,g,l;_.each(["freq","time"],function(k){if(d[k]){c.deleteTexture(d[k]);a.info.memory.textures--}e=d[k]=c.createTexture();a.info.memory.textures++;c.bindTexture(c.TEXTURE_2D,e);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_S,c.CLAMP_TO_EDGE);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_T,c.REPEAT);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_MIN_FILTER,c.LINEAR);c.texParameteri(c.TEXTURE_2D,
c.TEXTURE_MAG_FILTER,c.LINEAR);g=h[k];l=new Uint8Array(g.length*f);if(k=="time")for(k=0;k<g.length*f;++k)l[k]=128;c.texImage2D(c.TEXTURE_2D,0,c.ALPHA,g.length,f,0,c.ALPHA,c.UNSIGNED_BYTE,l)})},update:function(){var a=this.renderer.getContext(),c=this.textures,d=this.source,h=this.history,f=this.timeIndex,e=d.data;d.update();_.each(["freq","time"],function(d){a.bindTexture(a.TEXTURE_2D,c[d]);a.pixelStorei(a.UNPACK_ALIGNMENT,1);a.texSubImage2D(a.TEXTURE_2D,0,0,f,e[d].length,1,a.ALPHA,a.UNSIGNED_BYTE,
e[d])});this.emit("update",this.uniforms());this.timeIndex=(f+1)%h},get:function(){return this.textures},uniforms:function(){var a=this.data.levels,c=this.data.beat;return{audioIsBeat:c.is,audioWasBeat:c.was,audioLevels:a.direct,audioLevelsSmooth:a.smooth,audioLevelsChange:a.change,audioOffset:this.timeIndex/this.history,audioStep:{x:1/(this.source.samples-1),y:1/this.history}}}};MicroEvent.mixin(ThreeAudio.Textures);
ThreeAudio.GridGeometry=function(a,c,d,h,f){var e=a.source,h=(h||e.samples)-1,f=Math.min(a.history,f||a.history)-1,g=f/a.history,l=0.5/(e.samples-1);offsetV=0.5/a.history;a=new THREE.PlaneGeometry(c,d,h,f);_.each(a.faceVertexUvs[0],function(a){_.each(a,function(a){a.u=a.u+l;a.v=1-a.v*g+offsetV})});return a};ThreeAudio.LevelDetect=function(a){this.data=a;this.levels=[[0,0,0,0]];this.smooth=[0,0,0,0];this.change=[0,0,0,0];a.levels={direct:this.levels[0],smooth:this.smooth,change:this.change}};
ThreeAudio.LevelDetect.prototype.analyse=function(){for(var a=this.data,c=this.levels,d=this.smooth,h=this.change,f=[0,0,0,0],e=[a.time,a.filter.bass,a.filter.mid,a.filter.treble],a=0;a<4;++a){var g=f,l=a,k;k=e[a];for(var p=k.length,j=0,n=0;n<p;++n)var m=(k[n]-128)/128,j=j+m*m;k=Math.sqrt(j/p);g[l]=k}c.unshift(f);c.length>7&&c.pop();e=[1,2,1];g=4;l=Math.min(c.length,e.length);for(k=0;k<4;++k){for(a=f=0;a<l;++a)f=f+(c[a]&&c[a][k]||0)*e[a];d[k]=f/g}e=[1,3,2,-2,-3,-1];g=6;Math.min(c.length,e.length);
for(k=0;k<4;++k){for(a=f=0;a<l;++a)f=f+(c[a]&&c[a][k]||0)*e[a];h[k]=h[k]+(f/g-h[k])*0.5}this.data.levels.direct=this.levels[0]};
...
document.body.appendChild(this.t)},bpmToOffset:function(a){return this.n*a/3600},offsetToBPM:function(a){return 3600*a/this.n},analyse:function(){var a=this.data,c=a.levels,d=this.buffer,h=this.fft,f=this.n,e=this.history,g=this.histogram,l=this.histogramSorted,k=this,p=c.direct[0];for(e.unshift(p);e.length>f;)e.pop();d.set(e);h.forward(d);for(var j=h.real,e=h.real,h=h.imag,d=0;d<f;++d)j[d]=e[d]*e[d]+h[d]*h[d];e=0;for(d=2;d<f/8;++d)e=Math.max(e,j[d]);for(var h={},n=e/16,d=this.fMin+1;d<this.fMax;++d){var m=
j[d];if(m>n){var q=Math.max(j[d-1],j[d+1]);if(m>q){q=Math.min(j[d-1],j[d+1]);if(Math.abs(m-q)/m>0.5){var u=Math.sqrt(m/e);h[d]=u}}}}_.each(l,function(a){a.permanence=a.permanence*0.99;decay=Math.min(1,Math.log(1+a.permanence)/Math.log(10));a.strength=a.strength*decay;a.active=false});_.each(h,function(a,c){var c=+c,d=j[c-1],e=j[c+1],h=d+e-j[c]*2;b=(e-d)/2;fraction=-b/h;d=g[c];if(!d)if(d=g[c+1]){delete g[c+1];d.index=c;d.fraction=d.fraction+1;g[c]=d}else if(d=g[c-1]){delete g[c-1];d.index=c;d.fraction=
d.fraction-1;g[c]=d}else{d=g[c]={index:c,fraction:fraction,offset:0,strength:0,permanence:0,score:0,window:0};l.push(d)}d.fraction=d.fraction+(fraction-d.fraction)*0.1;d.strength=a;d.permanence=d.permanence+d.strength*0.01;d.offset=d.index+d.fraction;d.bpm=k.offsetToBPM(d.offset);d.window=f/d.offset;d.active=true});l.sort(function(a,c){return c.permanence-a.permanence});for(var r;(r=l[l.length-1])&&r.strength<0.01;){l.splice(l.length-1,1);delete g[r.index]}var v=this.fMin,s=0,t=0,o=null;_.each(l,
function(a){var c=0;if(!(a.offset<v)){var d,e=0,f=a.offset*1.5;_.each(l,function(c){if(c!=a&&!(c.offset<f)){var d=c.offset/a.offset,g=Math.max(0,1-Math.abs(d-Math.round(d))*8);u=c.strength*c.permanence*d;e=e+g*u}});d=e;d>c&&(c=d);c=c+a.strength*a.permanence;a.score=c;if(c>s){o=a;t=s;s=c}else c>t&&(t=c)}});if(o){o.score=s?1-t/s:0;o.confidence=Math.sqrt(o.score*o.permanence);if(this.beat)if(this.beat.confidence<o.confidence)this.beat=o;else{if(this.beat!=o)this.beat.confidence=this.beat.confidence*
...
3-a.beat.was)*0.33;this.debounceMaybe++;this.debouncePredict++;this.measure++;this.debug(c.direct,p,r,maybe,j,h,l,a.beat)},debug:function(a,c,d,h,f,e,g,l){var k=this,e=this.n;if(this.g){var p=["<strong>"+Math.round(l.bpm*10)/10+" BPM ("+Math.round(100*l.confidence)+"%) "+Math.round(l.permanence*100)+"</strong>"];_.each(g,function(a){var c=Math.round(k.offsetToBPM(a.offset)*10)/10;p.push([c," bpm - ",Math.round(a.offset*10)/10," ",Math.round(a.fraction*100)/100," - %: ",Math.round(a.strength*100),
" - p: ",Math.round(a.permanence*100)].join(""))});this.t.innerHTML=p.join("<br>");var j=this.g;j.fillStyle="#000000";j.fillRect(0,140,512,100);for(var n=0,m=2;m<e/8;++m)n=Math.max(f[m],n);n=1/n;j.beginPath();j.moveTo(0,200);for(m=2;m<e/8;++m)j.lineTo((m-2)*8,240-f[m]*n*100);j.strokeStyle="#ffffff";j.stroke();_.each(g,function(a){j.fillStyle="rgba(255,"+(a.active?"255":"0")+",0,"+(a.strength*0.5+0.5)+")";j.fillRect((a.offset-2)*8,140,1,100)});var m=this.i,q=0,f=function(a){a=Math.round(Math.max(0,
Math.min(255,a*255)));j.fillStyle="rgb("+[a,a,a].join()+")";j.fillRect(m,q,1,20);q=q+20};f(a[0]);f(a[1]);f(a[2]);f(a[3]);if(l.is){j.fillStyle=l.missed?"rgba(255,0,0,.5)":l.predicted?"rgba(255,180,0,.5)":"rgba(30,180,0,.5)";j.fillRect(this.i,0,1,100)}a=Math.round(Math.max(0,Math.min(255,l.was*255)));j.fillStyle="rgb("+a+","+a+","+a+")";j.fillRect(412,240,100,100);if(l.maybe){j.fillStyle=l.predicted?"rgba(100,0,230,.5)":"rgba(0,180,255,.5)";j.fillRect(this.i,0,1,100)}if(c){c=Math.floor(Math.max(0,Math.min(1,
c))*255);j.fillStyle="rgba(0,"+c+","+c+",1)";j.fillRect(this.i,80,1,20)}if(d){d=Math.floor(Math.max(0,Math.min(1,d))*255);j.fillStyle="rgba("+d+",0,"+d+",1)";j.fillRect(this.i,100,1,20)}if(h){h=Math.floor(Math.max(0,Math.min(1,h))*255);j.fillStyle="rgba("+h+","+h+",0,1)";j.fillRect(this.i,120,1,20)}this.i=(m+1)%512}}};tQuery.World.register("audio",function(a){return tQuery.createAudioSource(this,a)});
tQuery.register("createAudioSource",function(a,c){var d=new ThreeAudio.Source(c);d.textures=function(c){return tQuery.createAudioTextures(a,this,c)};return d});tQuery.register("createAudioTextures",function(a,c,d){var h=new ThreeAudio.Textures(a.tRenderer(),c,d);h.material=function(a,c,d,h,k){return tQuery.createAudioMaterial(this,a,c,d,h,k)};a.loop().hookPreRender(function(){h.update()});return h});
tQuery.register("createAudioMaterial",function(a,c,d,h,f,e){c=new ThreeAudio.Material(a,c,d,h,f,e);c.grid=function(c,d,e,f){return tQuery.createAudioGrid(a,c,d,e,f,this)};return c});tQuery.register("createAudioGrid",function(a,c,d,h,f,e){var g=[a,1,1,0,0];for(i in g)arguments[i]=arguments[i]||g[i];g=new ThreeAudio.GridGeometry(a,c||1,d||1,h,f);e=e||tQuery.defaultObject3DMaterial;g=new THREE.Mesh(g,e);g.doubleSided=true;g.frustumCulled=false;return tQuery(g)});
