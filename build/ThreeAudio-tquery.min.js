...
MicroEvent.mixin=function(b){for(var a=["bind","unbind","trigger","on","emit"],d=0;d<a.length;d++)b.prototype[a[d]]=MicroEvent.prototype[a[d]]};ThreeAudio.Source=function(b){this.fftSize=b||512;this.filters={};this.playing=false;this.init()};
...
ThreeAudio.Material=function(b,a,d,c,f,e){var e=e||[],f=_.extend(f||{},{audioLevels:{type:"fv1",value:0},audioLevelsSmooth:{type:"fv1",value:0},audioLevelsChange:{type:"fv1",value:0},audioOffset:{type:"f",value:0},audioStep:{type:"v2",value:{x:0,y:0}}}),h=0;_.each(b.get(),function(a,b){var c=new THREE.Texture(new Image,new THREE.UVMapping,THREE.ClampToEdgeWrapping,THREE.RepeatWrapping,THREE.NearestFilter,THREE.NearestFilter);c.__webglInit=true;c.__webglTexture=a;f[b+"Data"]={type:"t",texture:c,value:h++}});
_.each(c||[],function(a,b){f[b]={type:"t",value:h++,texture:a}});b.on("update",function(a){_.each(a,function(a,b){f[b].value=a})});return new THREE.ShaderMaterial({attributes:e,uniforms:f,vertexShader:ThreeAudio.getShader(a),fragmentShader:ThreeAudio.getShader(d)})};ThreeAudio.Textures=function(b,a,d){this.renderer=b;this.source=a;this.textures={};this.history=d||128;this.timeIndex=0;this.init()};
ThreeAudio.Textures.prototype={init:function(){var b=this.renderer,a=b.getContext(),d=this.textures,c=this.source,f=this.history,e,h,j;_.each(["freq","time"],function(g){if(d[g]){a.deleteTexture(d[g]);b.info.memory.textures--}e=d[g]=a.createTexture();b.info.memory.textures++;a.bindTexture(a.TEXTURE_2D,e);a.texParameteri(a.TEXTURE_2D,a.TEXTURE_WRAP_S,a.CLAMP_TO_EDGE);a.texParameteri(a.TEXTURE_2D,a.TEXTURE_WRAP_T,a.REPEAT);a.texParameteri(a.TEXTURE_2D,a.TEXTURE_MIN_FILTER,a.LINEAR);a.texParameteri(a.TEXTURE_2D,
a.TEXTURE_MAG_FILTER,a.LINEAR);h=c.data[g];j=new Uint8Array(h.length*f);if(g=="time")for(g=0;g<h.length*f;++g)j[g]=128;a.texImage2D(a.TEXTURE_2D,0,a.ALPHA,h.length,f,0,a.ALPHA,a.UNSIGNED_BYTE,j)});this.audioLevels=[[1,0,0,0]];this.audioLevelsSmooth=[0,0,0,0];this.audioLevelsChange=[0,0,0,0]},update:function(){function b(a){for(var b=a.length,c=0,d=0;d<b;++d)var e=(a[d]-128)/128,c=c+e*e;return c/b}var a=this.renderer.getContext(),d=this.textures,c=this.source,f=this.history,e=this.timeIndex,h=c.data;
levels=this.audioLevels;smooth=this.audioLevelsSmooth;change=this.audioLevelsChange;bins=[0,0,0,0];c.update();for(var j=[h.time,h.filter.bass,h.filter.mid,h.filter.treble],c=0;c<4;++c)bins[c]=Math.sqrt(b(j[c]));levels.unshift(bins);levels.length>7&&levels.pop();var g,l,m;g=[1,2,1];l=4;m=Math.min(levels.length,g.length);for(var k=0;k<4;++k){for(c=j=0;c<m;++c)j=j+(levels[c]&&levels[c][k]||0)*g[c];smooth[k]=j/l}g=[1,3,2,-2,-3,-1];l=6;Math.min(levels.length,g.length);for(k=0;k<4;++k){for(c=j=0;c<m;++c)j=
j+(levels[c]&&levels[c][k]||0)*g[c];change[k]=change[k]+(j/l-change[k])*0.3}_.each(["freq","time"],function(b){a.bindTexture(a.TEXTURE_2D,d[b]);a.pixelStorei(a.UNPACK_ALIGNMENT,1);a.texSubImage2D(a.TEXTURE_2D,0,0,e,h[b].length,1,a.ALPHA,a.UNSIGNED_BYTE,h[b])});this.emit("update",this.uniforms());this.timeIndex=(e+1)%f},get:function(){return this.textures},uniforms:function(){return{audioLevels:this.audioLevels[0],audioLevelsSmooth:this.audioLevelsSmooth,audioLevelsChange:this.audioLevelsChange,audioOffset:this.timeIndex/
this.history,audioStep:{x:1/(this.source.samples-1),y:1/this.history}}}};MicroEvent.mixin(ThreeAudio.Textures);ThreeAudio.GridGeometry=function(b,a,d,c,f){var e=b.source,c=(c||e.samples)-1,f=Math.min(b.history,f||b.history)-1,h=f/b.history,j=0.5/(e.samples-1);offsetV=0.5/b.history;b=new THREE.PlaneGeometry(a,d,c,f);_.each(b.faceVertexUvs[0],function(a){_.each(a,function(a){a.u=a.u+j;a.v=1-a.v*h+offsetV})});return b};tQuery.World.register("audio",function(b){return tQuery.createAudioSource(this,b)});
tQuery.register("createAudioSource",function(b,a){var d=new ThreeAudio.Source(a);d.textures=function(a){return tQuery.createAudioTextures(b,this,a)};return d});tQuery.register("createAudioTextures",function(b,a,d){var c=new ThreeAudio.Textures(b.tRenderer(),a,d);c.material=function(a,b,c,d,g){return tQuery.createAudioMaterial(this,a,b,c,d,g)};b.loop().hookPreRender(function(){c.update()});return c});
tQuery.register("createAudioMaterial",function(b,a,d,c,f,e){a=new ThreeAudio.Material(b,a,d,c,f,e);a.grid=function(a,c,d,e){return tQuery.createAudioGrid(b,a,c,d,e,this)};return a});tQuery.register("createAudioGrid",function(b){var a=ThreeAudio.GridGeometry,d=[b,1,1,0,0];for(i in d)arguments[i]=arguments[i]||d[i];return this._createMesh(a,d,arguments)});
