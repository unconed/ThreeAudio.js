...
ThreeAudio.Source.prototype={init:function(){var a=this.context=new webkitAudioContext;this.source=a.createBufferSource();this.analyser=a.createAnalyser();var c=this.analyser.fftSize=this.fftSize,e=this.filters;_.each({bass:{type:0,frequency:160,Q:1.2,gain:2},mid:{type:2,frequency:500,Q:1.2,gain:4},treble:{type:1,frequency:2E3,Q:1.2,gain:3}},function(d,g){var h=a.createBiquadFilter();h.key=g;h.type=d.type;h.frequency.value=d.frequency;h.Q.value=d.Q;h.analyser=a.createAnalyser();h.analyser.fftSize=
c;h.delayNode=a.createDelayNode();h.delayNode.delayTime.value=0;h.gainNode=a.createGainNode();h.gainNode.gain.value=d.gain;e[g]=h});this.delay=a.createDelayNode();this.delay.delayTime.value=this.fftSize*2/a.sampleRate;this.source.connect(this.analyser);this.analyser.connect(this.delay);this.delay.connect(a.destination);var d=this.source;_.each(e,function(a){d.connect(a.delayNode);a.delayNode.connect(a);a.connect(a.gainNode);a.gainNode.connect(a.analyser)});this.samples=this.analyser.frequencyBinCount;
...
this.levelDetect.analyse();this.beatDetect.analyse();return this},size:function(){return this.analyser.frequencyBinCount},load:function(a,c){var e=this.context,d=this.source,f=this,g=new XMLHttpRequest;g.open("GET",a,true);g.responseType="arraybuffer";g.onload=function(){var a=e.createBuffer(g.response,false);d.buffer=a;d.loop=true;f.playing&&f._play();c&&c()};g.send();return this},play:function(){this.playing=true;this.source.buffer&&this._play();return this},stop:function(){this.playing=false;this.source.buffer&&
...
ThreeAudio.Material=function(a,c,e,d,f,g){var g=g||[],f=_.extend(f||{},{audioIsBeat:{type:"f",value:0},audioWasBeat:{type:"f",value:0},audioLevels:{type:"fv1",value:[0,0,0,0]},audioLevelsSmooth:{type:"fv1",value:[0,0,0,0]},audioLevelsChange:{type:"fv1",value:[0,0,0,0]},audioOffset:{type:"f",value:0},audioStep:{type:"v2",value:{x:0,y:0}}}),h=0;_.each(a.get(),function(a,c){var d=new THREE.Texture(new Image,new THREE.UVMapping,THREE.ClampToEdgeWrapping,THREE.RepeatWrapping,THREE.NearestFilter,THREE.NearestFilter);
d.__webglInit=true;d.__webglTexture=a;f[c+"Data"]={type:"t",texture:d,value:h++}});_.each(d||[],function(a,c){f[c]={type:"t",value:h++,texture:ThreeAudio.toTexture(a)}});a.on("update",function(a){_.each(a,function(a,c){f[c].value=a})});return new THREE.ShaderMaterial({attributes:g,uniforms:f,vertexShader:ThreeAudio.getShader(c),fragmentShader:ThreeAudio.getShader(e)})};
...
ThreeAudio.Textures.prototype={init:function(){var a=this.renderer,c=a.getContext(),e=this.textures,d=this.data,f=this.history,g,h,k;_.each(["freq","time"],function(m){if(e[m]){c.deleteTexture(e[m]);a.info.memory.textures--}g=e[m]=c.createTexture();a.info.memory.textures++;c.bindTexture(c.TEXTURE_2D,g);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_S,c.CLAMP_TO_EDGE);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_T,c.REPEAT);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_MIN_FILTER,c.LINEAR);c.texParameteri(c.TEXTURE_2D,
c.TEXTURE_MAG_FILTER,c.LINEAR);h=d[m];k=new Uint8Array(h.length*f);if(m=="time")for(m=0;m<h.length*f;++m)k[m]=128;c.texImage2D(c.TEXTURE_2D,0,c.ALPHA,h.length,f,0,c.ALPHA,c.UNSIGNED_BYTE,k)})},update:function(){var a=this.renderer.getContext(),c=this.textures,e=this.source,d=this.history,f=this.timeIndex,g=e.data;e.update();_.each(["freq","time"],function(d){a.bindTexture(a.TEXTURE_2D,c[d]);a.pixelStorei(a.UNPACK_ALIGNMENT,1);a.texSubImage2D(a.TEXTURE_2D,0,0,f,g[d].length,1,a.ALPHA,a.UNSIGNED_BYTE,
g[d])});this.emit("update",this.uniforms());this.timeIndex=(f+1)%d},get:function(){return this.textures},uniforms:function(){var a=this.data.levels,c=this.data.beat;return{audioIsBeat:c.is,audioWasBeat:c.was,audioLevels:a.direct,audioLevelsSmooth:a.smooth,audioLevelsChange:a.change,audioOffset:this.timeIndex/this.history,audioStep:{x:1/(this.source.samples-1),y:1/this.history}}}};MicroEvent.mixin(ThreeAudio.Textures);
ThreeAudio.GridGeometry=function(a,c,e,d,f){var g=a.source,d=(d||g.samples)-1,f=Math.min(a.history,f||a.history)-1,h=f/a.history,k=0.5/(g.samples-1);offsetV=0.5/a.history;a=new THREE.PlaneGeometry(c,e,d,f);_.each(a.faceVertexUvs[0],function(a){_.each(a,function(a){a.u=a.u+k;a.v=1-a.v*h+offsetV})});return a};ThreeAudio.LevelDetect=function(a){this.data=a;this.levels=[[0,0,0,0]];this.smooth=[0,0,0,0];this.change=[0,0,0,0];a.levels={direct:this.levels[0],smooth:this.smooth,change:this.change}};
ThreeAudio.LevelDetect.prototype.analyse=function(){for(var a=this.data,c=this.levels,e=this.smooth,d=this.change,f=[0,0,0,0],g=[a.time,a.filter.bass,a.filter.mid,a.filter.treble],a=0;a<4;++a){var h=f,k=a,m;m=g[a];for(var j=m.length,p=0,u=0;u<j;++u)var s=(m[u]-128)/128,p=p+s*s;m=Math.sqrt(p/j);h[k]=m}c.unshift(f);c.length>7&&c.pop();g=[1,2,1];h=4;k=Math.min(c.length,g.length);for(m=0;m<4;++m){for(a=f=0;a<k;++a)f=f+(c[a]&&c[a][m]||0)*g[a];e[m]=f/h}g=[1,3,2,-2,-3,-1];h=6;Math.min(c.length,g.length);
for(m=0;m<4;++m){for(a=f=0;a<k;++a)f=f+(c[a]&&c[a][m]||0)*g[a];d[m]=d[m]+(f/h-d[m])*0.5}this.data.levels.direct=this.levels[0]};var __taDebug=!1;
...
document.body.appendChild(this.t)},bpmToOffset:function(a){return this.n*a/3600},offsetToBPM:function(a){return 3600*a/this.n},analyse:function(){var a=this.data,c=a.levels,e=this.buffer,d=this.fft,f=this.n,g=this.history,h=this.histogram,k=this.histogramSorted,m=this;a.beat.missed=false;a.beat.maybe=false;a.beat.is=false;a.beat.predicted=false;a.beat.locked=false;a.beat.adjusted=false;if(this.beat){a.beat.confidence=this.beat.confidence;a.beat.permanence=this.beat.permanence;a.beat.bpm=this.beat.bpm}var j=
c.direct[0];this.background=this.background+(j-this.background)*0.2;this.energy=this.energy+(j-this.energy)*0.4;this.signal=j=(this.energy-this.background)/(1-this.background)*3;c=this.maybe;maybe=j*2-this.signal;this.maxMaybe=Math.max(this.maxMaybe*0.99,maybe);this.maybe=maybe=maybe/Math.max(0.2,this.maxMaybe)-0.7;this.sample=j;for(g.unshift(j);g.length>f;)g.pop();e.set(g);d.forward(e);for(var p=this.spectrum=d.real,j=d.real,e=d.imag,d=0;d<f;++d)p[d]=j[d]*j[d]+e[d]*e[d];e=0;for(d=2;d<f/8;++d)e=Math.max(e,
p[d]);for(var g={},u=e/16,d=this.fMin+1;d<this.fMax;++d){j=p[d];if(j>u){var s=Math.max(p[d-1],p[d+1]);if(j>s){s=Math.min(p[d-1],p[d+1]);if(Math.abs(j-s)/j>0.5){var n=Math.sqrt(j/e);g[d]=n}}}}_.each(k,function(a){a.permanence=a.permanence*0.99;decay=Math.min(1,Math.log(1+a.permanence)/Math.log(10));a.strength=a.strength*decay;a.active=false});_.each(g,function(a,c){var c=+c,d=p[c-1],e=p[c+1],g=d+e-p[c]*2;b=(e-d)/2;fraction=-b/g;d=h[c];if(!d)if(d=h[c+1]){delete h[c+1];d.index=c;d.fraction=d.fraction+
1;h[c]=d}else if(d=h[c-1]){delete h[c-1];d.index=c;d.fraction=d.fraction-1;h[c]=d}else{d=h[c]={index:c,fraction:fraction,offset:0,strength:0,permanence:0,score:0,window:0};k.push(d)}d.fraction=d.fraction+(fraction-d.fraction)*0.1;d.strength=a;d.permanence=d.permanence+d.strength*0.01;d.offset=d.index+d.fraction;d.bpm=m.offsetToBPM(d.offset);d.window=f/d.offset;d.active=true});k.sort(function(a,c){return c.permanence-a.permanence});for(var o;(o=k[k.length-1])&&o.strength<0.01;){k.splice(k.length-1,
1);delete h[o.index]}var q=this.fMin,t=0,v=0,r=null;_.each(k,function(a){var c=0;if(!(a.offset<q)){var d,e=0,f=a.offset*1.5;_.each(k,function(c){if(c!=a&&!(c.offset<f)){var d=c.offset/a.offset,g=Math.max(0,1-Math.abs(d-Math.round(d))*4);n=c.strength*c.permanence*d;e=e+g*n}});d=e;d>c&&(c=d);c=c+a.strength*a.permanence;a.score=c;if(c>t){r=a;v=t;t=c}else c>v&&(v=c)}});if(r){r.score=t?1-v/t:0;r.confidence=Math.min(1,r.score+r.permanence);if(this.beat)if(this.beat.confidence<r.confidence)this.beat=r;else{if(this.beat!=
...
this.frames++;__taDebug&&this.debug()},debug:function(){function a(a){a=Math.round(Math.max(0,Math.min(255,a*255)));n.fillStyle="rgb("+[a,a,a].join()+")";n.fillRect(q,t,1,20);t=t+20}var c=this.data.levels.direct,e=this.sample,d=this.signal,f=this.maybe,g=this.spectrum,h=this.histogram,k=this.data.beat,m=this,j=this.n;if(this.beat){var p=this.beat,u=p.offset*1.5;_.each(h,function(a){var c=a==p?1:0;if(a.offset>u){c=a.offset/p.offset;c=Math.max(0,1-Math.abs(c-Math.round(c))*4)}a.match=c})}var s=["<strong><span"+
(k.locked?' style="color: rgb(180,255,0)"':"")+">"+Math.round(k.bpm*10)/10+" BPM </span> ("+Math.round(100*k.confidence)+"%) P:"+Math.round(k.permanence*100)+" \u00b5 = "+Math.round(this.mean*10)/10+" \u03c3 = "+Math.round(this.stddev*100)/100+"</strong> "+this.found+"f "+this.missed+"m"];_.each(h,function(a){var c=Math.round(m.offsetToBPM(a.offset)*10)/10;s.push([c," bpm - ",Math.round(a.strength*100),"% P: ",Math.round(a.permanence*100)].join(""))});this.t.innerHTML=s.join("<br>");var n=this.g;
n.fillStyle="#000000";n.fillRect(0,140,512,100);for(var o=0,q=2;q<j/8;++q)o=Math.max(g[q],o);o=1/o;n.beginPath();n.moveTo(0,200);for(q=2;q<j/8;++q)n.lineTo((q-2)*8,240-g[q]*o*100);n.strokeStyle="#ffffff";n.stroke();_.each(h,function(a){var c=a.strength*0.75+0.25,d=a.active?[Math.round(255-195*a.match),Math.round(180+40*a.match),0].join():"255,10,10";n.fillStyle="rgba("+d+","+c+")";n.fillRect((a.offset-2)*8,140,1,100)});var q=this.i,t=0;a(c[1]);a(c[2]);a(c[3]);a(0);if(k.is){n.fillStyle=k.missed?"rgba(255,0,0,.5)":
...
"rgba("+d+","+Math.round(d*0.8)+","+Math.round(d*0.5)+",1)";n.fillRect(this.i,100,1,20);f=k.is||k.maybe?Math.floor(Math.max(0,Math.min(1,f+0.5))*255):0;n.fillStyle="rgba("+Math.round(f*0.9)+","+f+","+Math.round(f*0.5)+",1)";n.fillRect(this.i,120,1,20);this.i=(q+1)%512}};
