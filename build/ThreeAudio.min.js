...
this.data={freq:new Uint8Array(this.samples),time:new Uint8Array(this.samples),filter:{bass:new Uint8Array(256),mid:new Uint8Array(256),treble:new Uint8Array(256)}}},update:function(){var a=this.analyser,b=this.data;a.smoothingTimeConstant=0;a.getByteFrequencyData(b.freq);a.getByteTimeDomainData(b.time);_.each(this.filters,function(a){a.analyser.getByteTimeDomainData(b.filter[a.key])});return this},size:function(){return this.analyser.frequencyBinCount},load:function(a,b){var c=this.context,k=this.source,
e=new XMLHttpRequest;e.open("GET",a,true);e.responseType="arraybuffer";e.onload=function(){var a=c.createBuffer(e.response,false);k.buffer=a;k.loop=true;this.playing&&this._play();b&&b()}.bind(this);e.send();return this},play:function(){this.playing=true;this.source.buffer&&this._play();return this},stop:function(){this.playing=false;this.source.buffer&&this._stop();return this},_play:function(){this.source.noteOn(0)},_stop:function(){this.source.noteOff(0)}};
ThreeAudio.Material=function(a,b,c,k,e,d){var d=d||[],e=_.extend(e||{},{audioLevels:{type:"fv1",value:0},audioLevelsSmooth:{type:"fv1",value:0},audioLevelsChange:{type:"fv1",value:0},audioOffset:{type:"f",value:0},audioStep:{type:"v2",value:{x:0,y:0}}}),f=0;_.each(a.get(),function(a,b){var c=new THREE.Texture(new Image,new THREE.UVMapping,THREE.ClampToEdgeWrapping,THREE.RepeatWrapping,THREE.NearestFilter,THREE.NearestFilter);c.__webglInit=true;c.__webglTexture=a;e[b+"Data"]={type:"t",texture:c,value:f++}});
_.each(k||[],function(a,b){e[b]={type:"t",value:f++,texture:a}});a.on("update",function(a){_.each(a,function(a,b){e[b].value=a})});return new THREE.ShaderMaterial({attributes:d,uniforms:e,vertexShader:ThreeAudio.getShader(b),fragmentShader:ThreeAudio.getShader(c)})};ThreeAudio.Textures=function(a,b,c){this.renderer=a;this.source=b;this.textures={};this.history=c||128;this.timeIndex=0;this.init()};
ThreeAudio.Textures.prototype={init:function(){var a=this.renderer,b=a.getContext(),c=this.textures,k=this.source,e=this.history,d,f,j;_.each(["freq","time"],function(g){if(c[g]){b.deleteTexture(c[g]);a.info.memory.textures--}d=c[g]=b.createTexture();a.info.memory.textures++;b.bindTexture(b.TEXTURE_2D,d);b.texParameteri(b.TEXTURE_2D,b.TEXTURE_WRAP_S,b.CLAMP_TO_EDGE);b.texParameteri(b.TEXTURE_2D,b.TEXTURE_WRAP_T,b.REPEAT);b.texParameteri(b.TEXTURE_2D,b.TEXTURE_MIN_FILTER,b.LINEAR);b.texParameteri(b.TEXTURE_2D,
b.TEXTURE_MAG_FILTER,b.LINEAR);f=k.data[g];j=new Uint8Array(f.length*e);if(g=="time")for(g=0;g<f.length*e;++g)j[g]=128;b.texImage2D(b.TEXTURE_2D,0,b.ALPHA,f.length,e,0,b.ALPHA,b.UNSIGNED_BYTE,j)});this.audioLevels=[[1,0,0,0]];this.audioLevelsSmooth=[0,0,0,0];this.audioLevelsChange=[0,0,0,0]},update:function(){var a=this.renderer.getContext(),b=this.textures,c=this.source,k=this.history,e=this.timeIndex,d=c.data;levels=this.audioLevels;smooth=this.audioLevelsSmooth;change=this.audioLevelsChange;bins=
[0,0,0,0];c.update();for(var f=[d.time,d.filter.bass,d.filter.mid,d.filter.treble],c=0;c<4;++c){var j=bins,g=c,l;l=f[c];for(var h=l.length,m=0,n=0;n<h;++n)var o=(l[n]-128)/128,m=m+o*o;l=Math.sqrt(m/h);j[g]=l}levels.unshift(bins);levels.length>7&&levels.pop();j=[1,2,1];g=4;l=Math.min(levels.length,j.length);for(h=0;h<4;++h){for(c=f=0;c<l;++c)f=f+(levels[c]&&levels[c][h]||0)*j[c];smooth[h]=f/g}j=[1,3,2,-2,-3,-1];g=6;Math.min(levels.length,j.length);for(h=0;h<4;++h){for(c=f=0;c<l;++c)f=f+(levels[c]&&
levels[c][h]||0)*j[c];change[h]=change[h]+(f/g-change[h])*0.3}_.each(["freq","time"],function(c){a.bindTexture(a.TEXTURE_2D,b[c]);a.pixelStorei(a.UNPACK_ALIGNMENT,1);a.texSubImage2D(a.TEXTURE_2D,0,0,e,d[c].length,1,a.ALPHA,a.UNSIGNED_BYTE,d[c])});this.emit("update",this.uniforms());this.timeIndex=(e+1)%k},get:function(){return this.textures},uniforms:function(){return{audioLevels:this.audioLevels[0],audioLevelsSmooth:this.audioLevelsSmooth,audioLevelsChange:this.audioLevelsChange,audioOffset:this.timeIndex/
this.history,audioStep:{x:1/(this.source.samples-1),y:1/this.history}}}};MicroEvent.mixin(ThreeAudio.Textures);ThreeAudio.GridGeometry=function(a,b,c,k,e){var d=a.source,k=(k||d.samples)-1,e=Math.min(a.history,e||a.history)-1,f=e/a.history,j=0.5/(d.samples-1);offsetV=0.5/a.history;a=new THREE.PlaneGeometry(b,c,k,e);_.each(a.faceVertexUvs[0],function(a){_.each(a,function(a){a.u=a.u+j;a.v=1-a.v*f+offsetV})});return a};
