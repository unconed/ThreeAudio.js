...
function FourierTransform(a,c){this.bufferSize=a;this.sampleRate=c;this.bandwidth=2/a*c/2;this.spectrum=new Float32Array(a/2);this.real=new Float32Array(a);this.imag=new Float32Array(a);this.peak=this.peakBand=0;this.getBandFrequency=function(a){return this.bandwidth*a+this.bandwidth/2};this.calculateSpectrum=function(){for(var c=this.spectrum,d=this.real,e=this.imag,h=2/this.bufferSize,g=Math.sqrt,k,m,n=0,q=a/2;n<q;n++)k=d[n],m=e[n],k=h*g(k*k+m*m),k>this.peak&&(this.peakBand=n,this.peak=k),c[n]=
k}}function DFT(a,c){FourierTransform.call(this,a,c);var j=a/2*a,d=2*Math.PI;this.sinTable=new Float32Array(j);this.cosTable=new Float32Array(j);for(var e=0;e<j;e++)this.sinTable[e]=Math.sin(e*d/a),this.cosTable[e]=Math.cos(e*d/a)}DFT.prototype.forward=function(a){for(var c=this.real,j=this.imag,d,e,h=0;h<this.bufferSize/2;h++){for(var g=e=d=0;g<a.length;g++)d+=this.cosTable[h*g]*a[g],e+=this.sinTable[h*g]*a[g];c[h]=d;j[h]=e}return this.calculateSpectrum()};
...
FFT.prototype.forward=function(a){var c=this.bufferSize,j=this.cosTable,d=this.sinTable,e=this.reverseTable,h=this.real,g=this.imag,k=Math.floor(Math.log(c)/Math.LN2);if(Math.pow(2,k)!==c)throw"Invalid buffer size, must be a power of 2.";if(c!==a.length)throw"Supplied buffer is not the same size as defined FFT. FFT Size: "+c+" Buffer Size: "+a.length;var k=1,m,n,q,p,t,f;for(f=0;f<c;f++)h[f]=a[e[f]],g[f]=0;for(;k<c;){a=j[k];e=d[k];m=1;for(var s=n=0;s<k;s++){for(f=s;f<c;)q=f+k,p=m*h[q]-n*g[q],t=m*g[q]+
n*h[q],h[q]=h[f]-p,g[q]=g[f]-t,h[f]+=p,g[f]+=t,f+=k<<1;f=m;m=f*a-n*e;n=f*e+n*a}k<<=1}return this.calculateSpectrum()};
FFT.prototype.inverse=function(a,c){var j=this.bufferSize,d=this.cosTable,e=this.sinTable,h=this.reverseTable,a=a||this.real,c=c||this.imag,g=1,k,m,n,q,p,t,f;for(f=0;f<j;f++)c[f]*=-1;k=new Float32Array(j);m=new Float32Array(j);for(f=0;f<a.length;f++)k[f]=a[h[f]],m[f]=c[h[f]];a=k;for(c=m;g<j;){h=d[g];k=e[g];m=1;for(var s=n=0;s<g;s++){for(f=s;f<j;)q=f+g,p=m*a[q]-n*c[q],t=m*c[q]+n*a[q],a[q]=a[f]-p,c[q]=c[f]-t,a[f]+=p,c[f]+=t,f+=g<<1;f=m;m=f*h-n*k;n=f*k+n*h}g<<=1}d=new Float32Array(j);for(f=0;f<j;f++)d[f]=
a[f]/j;return d};
function RFFT(a,c){FourierTransform.call(this,a,c);this.trans=new Float32Array(a);this.reverseTable=new Uint32Array(a);this.reverseBinPermute=function(a,c){var e=this.bufferSize,h=e>>>1,e=e-1,g=1,k=0,m;a[0]=c[0];do{k+=h;a[g]=c[k];a[k]=c[g];g++;for(m=h<<1;m>>=1,!((k^=m)&m););k>=g&&(a[g]=c[k],a[k]=c[g],a[e-g]=c[e-k],a[e-k]=c[e-g]);g++}while(g<h);a[e]=c[e]};this.generateReverseTable=function(){var a=this.bufferSize,c=a>>>1,a=a-1,e=1,h=0,g;this.reverseTable[0]=0;do{h+=c;this.reverseTable[e]=h;this.reverseTable[h]=
e;e++;for(g=c<<1;g>>=1,!((h^=g)&g););h>=e&&(this.reverseTable[e]=h,this.reverseTable[h]=e,this.reverseTable[a-e]=a-h,this.reverseTable[a-h]=a-e);e++}while(e<c);this.reverseTable[a]=a};this.generateReverseTable()}
RFFT.prototype.forward=function(a){var c=this.bufferSize,j=this.spectrum,d=this.trans,e=2*Math.PI,h=Math.sqrt,g=c>>>1,k=2/c,m,n,q,p,t,f,s,r,x,v,u,C,D,E,G,z,A,B,H,I,J;this.reverseBinPermute(d,a);p=0;for(var y=4;p<c;y*=4){for(var w=p;w<c;w+=y)z=d[w]-d[w+1],d[w]+=d[w+1],d[w+1]=z;p=2*(y-1)}a=2;for(q=c>>>1;q>>>=1;){p=0;a<<=1;y=a<<1;m=a>>>2;n=a>>>3;do{if(1!==m)for(w=p;w<c;w+=y)r=w,x=r+m,v=x+m,u=v+m,p=d[v]+d[u],d[u]-=d[v],d[v]=d[r]-p,d[r]+=p,r+=n,x+=n,v+=n,u+=n,p=d[v]+d[u],t=d[v]-d[u],p=-p*Math.SQRT1_2,
...
p,d[r]+=p,d[x]=s+d[C],d[C]-=s;p=(y<<1)-a;y<<=2}while(p<c)}}for(;--g;)e=d[g],a=d[c-g-1],e=k*h(e*e+a*a),e>this.peak&&(this.peakBand=g,this.peak=e),j[g]=e;j[0]=k*d[0];return j};(function(a){for(i in a)if(!window[i])throw"Error: ThreeAudio requires "+a[i];})({THREE:"Three.js",MicroEvent:"MicroEvent.js"});window.ThreeAudio=window.ThreeAudio||{};ThreeAudio.getShader=function(a){var c=document.getElementById(a);return c&&(c.innerText||c.textContent)||a};window._=window._||{};
...
ThreeAudio.Source.prototype={init:function(){var a=this.context=new webkitAudioContext;this.source=a.createBufferSource();this.analyser=a.createAnalyser();var c=this.analyser.fftSize=this.fftSize,j=this.filters;_.each({bass:{type:0,frequency:160,Q:1.2,gain:2},mid:{type:2,frequency:500,Q:1.2,gain:4},treble:{type:1,frequency:2E3,Q:1.2,gain:3}},function(d,h){var g=a.createBiquadFilter();g.key=h;g.type=d.type;g.frequency.value=d.frequency;g.Q.value=d.Q;g.analyser=a.createAnalyser();g.analyser.fftSize=
c;g.delayNode=a.createDelayNode();g.delayNode.delayTime.value=0;g.gainNode=a.createGainNode();g.gainNode.gain.value=d.gain;j[h]=g});this.delay=a.createDelayNode();this.delay.delayTime.value=2*this.fftSize/a.sampleRate;this.source.connect(this.analyser);this.analyser.connect(this.delay);this.delay.connect(a.destination);var d=this.source;_.each(j,function(a){d.connect(a.delayNode);a.delayNode.connect(a);a.connect(a.gainNode);a.gainNode.connect(a.analyser)});this.samples=this.analyser.frequencyBinCount;
...
ThreeAudio.Material=function(a,c,j,d,e,h){h=h||[];e=_.extend(e||{},{audioIsBeat:{type:"f",value:0},audioWasBeat:{type:"f",value:0},audioLevels:{type:"fv1",value:[0,0,0,0]},audioLevelsSmooth:{type:"fv1",value:[0,0,0,0]},audioLevelsChange:{type:"fv1",value:[0,0,0,0]},audioOffset:{type:"f",value:0},audioStep:{type:"v2",value:{x:0,y:0}}});_.each(a.get(),function(a,c){var d=new THREE.Texture(new Image,new THREE.UVMapping,THREE.ClampToEdgeWrapping,THREE.RepeatWrapping,THREE.NearestFilter,THREE.NearestFilter);
d.__webglInit=!0;d.__webglTexture=a;e[c+"Data"]={type:"t",value:d}});_.each(d||[],function(a,c){e[c]={type:"t",value:ThreeAudio.toTexture(a)}});a.on("update",function(a){_.each(a,function(a,c){e[c].value=a})});return new THREE.ShaderMaterial({attributes:h,uniforms:e,vertexShader:ThreeAudio.getShader(c),fragmentShader:ThreeAudio.getShader(j)})};ThreeAudio.Textures=function(a,c,j){this.renderer=a;this.source=c;this.textures={};this.history=j||128;this.timeIndex=0;this.data=c.data;this.init()};
ThreeAudio.Textures.prototype={init:function(){var a=this.renderer,c=a.getContext(),j=this.textures,d=this.data,e=this.history,h,g,k;_.each(["freq","time"],function(m){j[m]&&(c.deleteTexture(j[m]),a.info.memory.textures--);h=j[m]=c.createTexture();a.info.memory.textures++;c.bindTexture(c.TEXTURE_2D,h);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_S,c.CLAMP_TO_EDGE);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_T,c.REPEAT);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_MIN_FILTER,c.LINEAR);c.texParameteri(c.TEXTURE_2D,
...
ThreeAudio.GridGeometry=function(a,c,j,d,e){var h=a.source,d=(d||h.samples)-1,e=Math.min(a.history,e||a.history)-1,g=e/a.history,k=0.5/(h.samples-1);offsetV=0.5/a.history;a=new THREE.PlaneGeometry(c,j,d,e);_.each(a.faceVertexUvs[0],function(a){_.each(a,function(a){a.u+=k;a.v=1-a.v*g+offsetV})});return a};ThreeAudio.LevelDetect=function(a){this.data=a;this.levels=[[0,0,0,0]];this.smooth=[0,0,0,0];this.change=[0,0,0,0];a.levels={direct:this.levels[0],smooth:this.smooth,change:this.change}};
ThreeAudio.LevelDetect.prototype.analyse=function(){for(var a=this.data,c=this.levels,j=this.smooth,d=this.change,e=[0,0,0,0],h=[a.time,a.filter.bass,a.filter.mid,a.filter.treble],a=0;4>a;++a){var g=e,k=a,m;m=h[a];for(var n=m.length,q=0,p=0;p<n;++p)var t=(m[p]-128)/128,q=q+t*t;m=Math.sqrt(q/n);g[k]=m}c.unshift(e);7<c.length&&c.pop();h=[1,2,1];g=4;k=Math.min(c.length,h.length);for(m=0;4>m;++m){for(a=e=0;a<k;++a)e+=(c[a]&&c[a][m]||0)*h[a];j[m]=e/g}h=[1,3,2,-2,-3,-1];g=6;Math.min(c.length,h.length);
for(m=0;4>m;++m){for(a=e=0;a<k;++a)e+=(c[a]&&c[a][m]||0)*h[a];d[m]+=0.5*(e/g-d[m])}this.data.levels.direct=this.levels[0]};var __taDebug=!1;
...
"absolute";this.t.style.zIndex=20;this.t.style.marginTop="350px";document.body.appendChild(this.c);document.body.appendChild(this.t)},bpmToOffset:function(a){return this.n*a/3600},offsetToBPM:function(a){return 3600*a/this.n},analyse:function(){var a=this.data,c=a.levels,j=this.buffer,d=this.fft,e=this.n,h=this.history,g=this.histogram,k=this.histogramSorted,m=this;a.beat.missed=!1;a.beat.maybe=!1;a.beat.is=!1;a.beat.predicted=!1;a.beat.locked=!1;a.beat.adjusted=!1;this.beat?(a.beat.confidence=this.beat.confidence,
a.beat.permanence=this.beat.permanence,a.beat.bpm=this.beat.bpm):(a.beat.confidence=0,a.beat.permanence=0,a.beat.bpm="");var n=c.direct[0];this.background+=0.2*(n-this.background);this.energy+=0.4*(n-this.energy);this.signal=n=3*((this.energy-this.background)/(1-this.background));c=this.maybe;maybe=n;this.maxMaybe=Math.max(0.99*this.maxMaybe,maybe);this.maybe=maybe=maybe/Math.max(0.2,this.maxMaybe)-0.7;this.sample=n;for(h.unshift(n);h.length>e;)h.pop();j.set(h);d.forward(j);for(var q=this.spectrum=
d.real,n=d.real,j=d.imag,d=0;d<e;++d)q[d]=n[d]*n[d]+j[d]*j[d];j=0;for(d=2;d<e/8;++d)j=Math.max(j,q[d]);for(var h={},p=j/16,d=this.fMin+1;d<this.fMax;++d)if(n=q[d],n>p){var t=Math.max(q[d-1],q[d+1]);if(n>t&&(t=Math.min(q[d-1],q[d+1]),0.5<Math.abs(n-t)/n)){var f=Math.sqrt(n/j);h[d]=f}}_.each(k,function(a){a.permanence*=0.99;decay=Math.min(1,Math.log(1+a.permanence)/Math.log(10));a.strength*=decay;a.active=!1});_.each(h,function(a,c){var c=+c,d=q[c-1],f=q[c+1],h=d+f-2*q[c];b=(f-d)/2;fraction=-b/h;d=
...
f.fillStyle="rgb("+c+","+c+","+c+")";f.fillRect(342,270,170,30);k.maybe&&!k.is&&(f.fillStyle="rgba(64,64,64,.75)",f.fillRect(this.i,120,2,40));j=Math.floor(255*Math.max(0,Math.min(1,1.4*j+0.5)));f.fillStyle="rgba("+Math.round(0.2*j)+","+Math.round(0.6*j)+","+j+",1)";f.fillRect(this.i,60,1,20);d=Math.floor(255*Math.max(0,Math.min(1,2*d)));f.fillStyle="rgba("+Math.round(0.2*d)+","+Math.round(d)+","+Math.round(0.5*d)+",1)";f.fillRect(this.i,80,1,20);e=k.is||k.maybe?Math.floor(255*Math.max(0,Math.min(1,
