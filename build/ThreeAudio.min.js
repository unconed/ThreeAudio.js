...
ThreeAudio.Source=function(a){"number"==typeof a&&(a={fftSize:a});a=_.extend({fftSize:1024,detectors:[ThreeAudio.LevelDetect,ThreeAudio.BeatDetect]},a);this.fftSize=a.fftSize;this.filters={};this.playing=!1;this.processingDelay=0;if(webkitAudioContext||AudioContext)this.initElement(a.element);else throw"Web Audio API not supported";};
...
ThreeAudio.LevelDetect.prototype.analyse=function(){for(var a=this.data,c=this.levels,f=this.smooth,e=this.change,d=[0,0,0,0],n=[a.time,a.filter.bass,a.filter.mid,a.filter.treble],a=0;4>a;++a){var m=d,h=a,k;k=n[a];for(var s=k.length,p=0,q=0;q<s;++q)var t=(k[q]-128)/128,p=p+t*t;k=Math.sqrt(p/s);m[h]=k}c.unshift(d);7<c.length&&c.pop();n=[1,2,1];m=4;h=Math.min(c.length,n.length);for(k=0;4>k;++k){for(a=d=0;a<h;++a)d+=(c[a]&&c[a][k]||0)*n[a];f[k]=d/m}n=[1,3,2,-2,-3,-1];m=6;Math.min(c.length,n.length);
