...
FFT.prototype.forward=function(a){var c=this.bufferSize,j=this.cosTable,d=this.sinTable,e=this.reverseTable,h=this.real,f=this.imag,k=Math.floor(Math.log(c)/Math.LN2);if(Math.pow(2,k)!==c)throw"Invalid buffer size, must be a power of 2.";if(c!==a.length)throw"Supplied buffer is not the same size as defined FFT. FFT Size: "+c+" Buffer Size: "+a.length;var k=1,m,n,q,p,t,g;for(g=0;g<c;g++)h[g]=a[e[g]],f[g]=0;for(;k<c;){a=j[k];e=d[k];m=1;for(var s=n=0;s<k;s++){for(g=s;g<c;)q=g+k,p=m*h[q]-n*f[q],t=m*f[q]+
n*h[q],h[q]=h[g]-p,f[q]=f[g]-t,h[g]+=p,f[g]+=t,g+=k<<1;g=m;m=g*a-n*e;n=g*e+n*a}k<<=1}return this.calculateSpectrum()};
FFT.prototype.inverse=function(a,c){var j=this.bufferSize,d=this.cosTable,e=this.sinTable,h=this.reverseTable,a=a||this.real,c=c||this.imag,f=1,k,m,n,q,p,t,g;for(g=0;g<j;g++)c[g]*=-1;k=new Float32Array(j);m=new Float32Array(j);for(g=0;g<a.length;g++)k[g]=a[h[g]],m[g]=c[h[g]];a=k;for(c=m;f<j;){h=d[f];k=e[f];m=1;for(var s=n=0;s<f;s++){for(g=s;g<j;)q=g+f,p=m*a[q]-n*c[q],t=m*c[q]+n*a[q],a[q]=a[g]-p,c[q]=c[g]-t,a[g]+=p,c[g]+=t,g+=f<<1;g=m;m=g*h-n*k;n=g*k+n*h}f<<=1}d=new Float32Array(j);for(g=0;g<j;g++)d[g]=
a[g]/j;return d};
...
"absolute";this.t.style.zIndex=20;this.t.style.marginTop="350px";document.body.appendChild(this.c);document.body.appendChild(this.t)},bpmToOffset:function(a){return this.n*a/3600},offsetToBPM:function(a){return 3600*a/this.n},analyse:function(){var a=this.data,c=a.levels,j=this.buffer,d=this.fft,e=this.n,h=this.history,f=this.histogram,k=this.histogramSorted,m=this;a.beat.missed=!1;a.beat.maybe=!1;a.beat.is=!1;a.beat.predicted=!1;a.beat.locked=!1;a.beat.adjusted=!1;this.beat?(a.beat.confidence=this.beat.confidence,
...
l;n/=l;this.mean=s;this.stddev=Math.sqrt(n-s*s)}this.jitter=(this.stddev+0.5*this.missed)*(1+this.missed);this.decay+=0.4*(2.5*+a.beat.is-this.decay);a.beat.was+=0.4*(2.5*this.decay-a.beat.was);this.debounceMaybe++;this.debouncePredict++;this.measure++;this.frames++;__taDebug&&this.debug()},debug:function(){function a(a){a=Math.round(Math.max(0,Math.min(255,255*a)));g.fillStyle="rgb("+[a,a,a].join()+")";g.fillRect(r,x,1,20);x+=20}var c=this.data.levels.direct,j=this.sample,d=this.signal,e=this.maybe,
h=this.spectrum,f=this.histogram,k=this.data.beat,m=this,n=this.n;if(this.beat){var q=this.beat,p=1.5*q.offset;_.each(f,function(a){var c=a==q?1:0;a.offset>p&&(c=a.offset/q.offset,c=Math.max(0,1-4*Math.abs(c-Math.round(c))));a.match=c})}var t=["<strong><span"+(k.locked?' style="color: rgb(180,255,0)"':"")+">"+Math.round(10*k.bpm)/10+" BPM </span> ("+Math.round(100*k.confidence)+"%) P:"+Math.round(100*k.permanence)+" \u00b5 = "+Math.round(10*this.mean)/10+" \u03c3 = "+Math.round(100*this.stddev)/100+
"</strong> "+this.found+"f "+this.missed+"m"];_.each(f,function(a){var c=Math.round(10*m.offsetToBPM(a.offset))/10;t.push([c," bpm - ",Math.round(100*a.strength),"% P: ",Math.round(100*a.permanence)].join(""))});this.t.innerHTML=t.join("<br>");var g=this.g;g.fillStyle="#000000";g.fillRect(0,169,512,102);for(var s=0,r=2;r<n/8;++r)s=Math.max(h[r],s);s=1/s;g.beginPath();g.moveTo(0,270);for(r=2;r<n/8;++r)g.lineTo(8*(r-2),270-100*h[r]*s);g.strokeStyle="#ffffff";g.stroke();_.each(f,function(a){var c=0.75*
a.strength+0.25,d=a.active?[Math.round(255-195*a.match),Math.round(180+40*a.match),0].join():"255,10,10";g.fillStyle="rgba("+d+","+c+")";g.fillRect(8*(a.offset-2),170,1,100)});var r=this.i,x=0;a(c[1]);a(c[2]);a(c[3]);a(0);a(0);a(0);a(0);a(0);g.fillStyle="rgba(160,160,255,.85)";g.fillRect(r+1,0,2,120);k.is&&(g.fillStyle=k.missed?"rgba(255,0,0,.7)":k.maybe?"rgba(0,180,255,.9)":k.predicted?"rgba(255,200,0,1)":"rgba(60,220,0,1)",g.fillRect(this.i,120,2,40));c=Math.round(Math.max(0,Math.min(255,255*k.was)));
g.fillStyle="rgb("+c+","+c+","+c+")";g.fillRect(342,270,170,30);k.maybe&&!k.is&&(g.fillStyle="rgba(64,64,64,.75)",g.fillRect(this.i,120,2,40));j=Math.floor(255*Math.max(0,Math.min(1,1.4*j+0.5)));g.fillStyle="rgba("+Math.round(0.2*j)+","+Math.round(0.6*j)+","+j+",1)";g.fillRect(this.i,60,1,20);d=Math.floor(255*Math.max(0,Math.min(1,2*d)));g.fillStyle="rgba("+Math.round(0.2*d)+","+Math.round(d)+","+Math.round(0.5*d)+",1)";g.fillRect(this.i,80,1,20);e=k.is||k.maybe?Math.floor(255*Math.max(0,Math.min(1,
1.2*e+0.5))):0;g.fillStyle="rgba("+Math.round(e)+","+e+","+Math.round(e)+",1)";g.fillRect(this.i,100,1,20);this.i=(r+1)%512}};
